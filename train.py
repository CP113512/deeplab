...
from utils.utils import download_weights, show_config, get_lr
...
# resume = True
# ------------------------------------------------------------------#
#   args.data_path  数据集路径
# ------------------------------------------------------------------#
# args.data_path  = '../three_datasets-AOP/datasets-cp'
# ---------------------------------------------------------------------#
...
# ----------------------------------------------------------------------------------------------------------------------------#
# model_path      = "logs/cp-aop-5/best_epoch_weights.pth"
# ---------------------------------------------------------#
...
def main(args):
    ...
    if args.model_path != '':
        ...
        # lr_scheduler_func.load_state_dict(checkpoint['lr_scheduler'])
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(args.data_path, "Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(args.data_path, "Segmentation/val.txt"),"r") as f:
    ...
    if True:
        ...
        train_dataset   = DeeplabDataset(train_lines, input_shape, args.num_classes, True, args.data_path)
        val_dataset     = DeeplabDataset(val_lines, input_shape, args.num_classes, False, args.data_path)
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, args.num_classes, val_lines, args.data_path, log_dir, args.cuda, \
        ...
        for epoch in range(args.Init_Epoch, args.UnFreeze_Epoch):
            ...
            wandb.watch(model)
            lr = get_lr(optimizer)
            wandb.log({'train_loss': loss_history.losses[-1],
